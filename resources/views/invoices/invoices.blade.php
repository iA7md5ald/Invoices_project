...
@section('content')
    ...
    @endif
				...
				<div class="row">
                                                        ...
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu tx-13">
                                                                ...
                                                                <a class="dropdown-item" href="#" data-invoice_id="{{ $invoice->id }}" data-toggle="modal" data-target="#delete_invoice">حذف الفاتورة</a>
                                                                    ...
                                                                    @can('تغير حالة الدفع')
                                                                <a class="dropdown-item"
                                                                   href=" {{ url('show_status') }}/{{ $invoice->id}}">تعديل حالة الدفع</a>
                                                                    @endcan
                                                                    @can('ارشفة الفاتورة')
                                                                <a class="dropdown-item" href="#" data-invoice_id="{{ $invoice->id }}" data-toggle="modal" data-target="#move_archive">&nbsp;&nbsp;نقل الي الارشيف</a>
                                                                    @endcan
                                                                    @can('طباعةالفاتورة')
                                                                <a class="dropdown-item"
                                                                   href=" {{ url('print') }}/{{ $invoice->id}}">طباعة الفاتورة</a>
                                                                    @endcan
                                                            </div>
                                                    </td>
                                                </tr>
                    ...
                    <div class="modal fade" id="move_archive" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                         aria-hidden="true">
...
@section('js')
    ...
    <script>
        $('#move_archive').on('show.bs.modal',function (event) {
            var button =$(event.relatedTarget)
            var invoice_id = button.data('invoice_id')
            var modal = $(this)
            modal.find('.modal-body #invoice_id').val(invoice_id)
